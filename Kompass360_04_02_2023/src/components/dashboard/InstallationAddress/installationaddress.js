...
const API = `${process.env.REACT_APP_API_BASE_URL}InstallationAddress/GetAllInstallationAddresses`;
...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    ...
    let filteredData = tempData.filter(
      (x) =>
        ...
        x.addressName.toLowerCase().includes(lsearchText) ||
        x.latitude.includes(lsearchText) ||
        x.longitude.includes(lsearchText) ||
  ...
  const handleModal = async (id) => {
    ...
    let result = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}InstallationAddress/GetInstallationAddressByID?installationAddressID=${id}`
    );
    const option = {
      replace: true,
      state: {
        mode: Modes.edit,
        installationAddressData: result.data,
      },
    };
    navigate(routeNames.CREATEINSTALLATIONADDRESS, option);
  };
  ...
  const getinspectionResponsibles = (id) => {
  ...
  const deleteInstallationAddress = (installationAddressID) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}InstallationAddress/DeleteInstallationAddress?installationAddressID=${installationAddressID}`
          )
          .then((res) => {
            Swal.fire(res.data);
            getApiData();
          ...
          .catch(() => {
            Swal.fire("Installation Address not deleted.");
          });
      }
    });
  };
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <button
          ...
          onClick={() =>
            navigate(routeNames.CREATEINSTALLATIONADDRESS, {
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      ...
                      tabIndex={-1}
                      key={row.installationAddressID}
                      selected={isItemSelected}
                    >
                      <TableCell align="center">{row.title}</TableCell>
                      <TableCell align="center">{row.description}</TableCell>
                      <TableCell align="center">{row.addressName}</TableCell>
                      ...
                      <TableCell>
                        <div style={{ display: "flex" }}>
                          ...
                          btn-primary btn-sm text-center"
                              ...
                              onClick={() =>
                                handleModal(row.installationAddressID)
                              }
                            >
                              <EditIcon />
                            </button>
                          ...
                          btn-danger btn-sm"
                            style={{ margin: 3 }}
                            onClick={() =>
                              deleteInstallationAddress(
                                row.installationAddressID
                              )
                            }
                          >
                            <DeleteTwoToneIcon />
