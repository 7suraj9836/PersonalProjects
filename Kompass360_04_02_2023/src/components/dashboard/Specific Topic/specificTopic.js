...
const API = `${process.env.REACT_APP_API_BASE_URL}SpecificTopic/GetAllSpecificTopics`;
...
const headCells = [
  ...
  {
    id: "relatedComponentTypeName",
    numeric: true,
    disablePadding: false,
    label: "RelatedComponentType",
  },
  {
    id: "listNumber",
    numeric: true,
...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    let tempData = baseApiData.map((ele) => {
      ...
      if (!ele.relatedComponentTypeName) ele.relatedComponentTypeName = "";
    ...
    let filteredData = tempData.filter(
      (x) =>
        ...
        x.relatedComponentTypeName.toLowerCase().includes(lsearchText) ||
  ...
  const handleModal = async (id) => {
    ...
    let result = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}SpecificTopic/GetSpecificTopicByID?specificTopicID=${id}`
    );
    const option = {
      replace: true,
      state: {
        mode: Modes.edit,
        specificTopicData: result.data,
      },
    };
    navigate(routeNames.CREATESPECIFICTOPIC, option);
  };
  ...
  const deleteCase = (specificTopicID) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}SpecificTopic/DeleteSpecificTopic?specificTopicID=${specificTopicID}`
          )
          .then((res) => {
            Swal.fire(res.data);
            getApiData();
          ...
          .catch(() => {
            Swal.fire("Specific Topic not deleted.");
          });
      }
    });
  };
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <button
          ...
          onClick={() =>
            navigate(routeNames.CREATESPECIFICTOPIC, {
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      hover
                      onClick={(event) =>
                        handleClick(event, row.specificTopicID)
                      }
                      role="checkbox"
                      aria-checked={isItemSelected}
                      tabIndex={-1}
                      key={row.specificTopicID}
                    ...
                    >
                      ...
                      {/* {console.log(competenceRequirementsData)} */}
                      <TableCell align="center">
                        {row.relatedComponentTypeName}
                      </TableCell>{" "}
                      ...
                      <TableCell align="center">{row.listNumber}</TableCell>
                      {/* <TableCell align="center">
                      ...
                      <TableCell>
                        <div style={{ display: "flex" }}>
                          ...
                          btn-primary btn-sm"
                              ...
                              onClick={() => handleModal(row.specificTopicID)}
                            >
                              <EditIcon />
                            </button>
                          </div>
                          ...
                          btn-danger btn-sm"
                            style={{ margin: 3 }}
                            onClick={() => deleteCase(row.specificTopicID)}
                          >
                            <DeleteTwoToneIcon />
                          </button>
                        </div>
