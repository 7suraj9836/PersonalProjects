...
const API = `${process.env.REACT_APP_API_BASE_URL}SubjectType/GetAllSubjectTypes`;
...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    let tempData = baseApiData.map((ele) => {
      ...
      if (!ele.relatedAreaName) ele.relatedAreaName = "";
    ...
    let filteredData = tempData.filter(
      (x) =>
        ...
        x.relatedAreaName.toLowerCase().includes(lsearchText) ||
  ...
  const handleModal = async (id) => {
    ...
    let result = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}SubjectType/GetSubjectTypeByID?SubjectTypeID=${id}`
    );
    const option = {
      replace: true,
      state: {
        mode: Modes.edit,
        subjectTypeData: result.data,
      },
    };
    navigate(routeNames.CREATESUBJECTTYPE, option);
  };
  ...
  const deleteCase = (subjectTypeID) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}SubjectType/DeleteSubjectType?subjectTypeID=${subjectTypeID}`
          )
          .then((res) => {
            Swal.fire(res.data);
            getApiData();
          ...
          .catch(() => {
            Swal.fire("SubjectType not deleted.");
          });
      }
    });
  };
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <button
          ...
          onClick={() =>
            navigate(routeNames.CREATESUBJECTTYPE, {
                      ...
                      <TableCell>
                        <div style={{ display: "flex" }}>
                          ...
                          btn-primary btn-sm"
                              ...
                              onClick={() => handleModal(row.subjectTypeID)}
                            >
                              <EditIcon />
                            </button>
                          </div>
                          ...
                          btn-danger btn-sm"
                            style={{ margin: 3 }}
                            onClick={() => deleteCase(row.subjectTypeID)}
                          >
                            <DeleteTwoToneIcon />
                          </button>
                        </div>
